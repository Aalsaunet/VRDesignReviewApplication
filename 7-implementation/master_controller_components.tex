...
What keys this correspond to is configured in the Unity Input Manager, which is a build specific configuration that allows the user to customize the 
...
For input keys not covered by \texttt{Input.GetAxis}, \texttt{Input.GetButton} is used. This function also uses the Unity Input Manager configuration, so the 
end user easily can configure the key layout. 
